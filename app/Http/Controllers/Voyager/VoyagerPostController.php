...
class VoyagerPostController extends VoyagerBaseController
{
...
//        dd($request);
    ...
    public function store(Request $request)
    {
        ...
        $text = $request->input('excerpt');
        $link = $request->input('slug');
        $text = strip_tags(mb_strimwidth($text, 0, 20, "..."), '<p></p>');
        $inline_button1 = array("text"=>"Посилання","url"=>"https://kafis.dp.ua/article/".$link);
        $inline_keyboard = [[$inline_button1]];
        $keyboard=array("inline_keyboard"=>$inline_keyboard);
        Telegram::sendPhoto([
            'chat_id' => '-1001471773809',
            'photo' => InputFile::createFromContents(file_get_contents($photo->getRealPath()), str_random(10) . '.' . $photo->getClientOriginalExtension()),
            'caption' => str_limit(strip_tags($text), 200, "...."),
